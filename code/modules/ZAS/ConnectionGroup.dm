...
/connection_edge/proc/flow(list/movable, differential, repelled)
	for(var/i = 1; i <= movable.len; i++)
		...
		if(M.last_airflow > world.time - A.config.airflow_delay) continue
		if(M.airflow_speed) continue

		//Check for knocking people over
		if(ismob(M) && differential > A.config.airflow_stun_pressure)
...
/connection_edge/zone/tick()
	...
	if(abs(differential) >= A.config.airflow_lightest_pressure)
		var/list/attracted
		var/list/repelled
		if(differential > 0)
